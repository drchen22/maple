...
const char *maple::opengl_errno_name(GLenum err) {
    switch (err) {
#define PER_GL_ERROR(x) case GL_##x: return #x;
    ...
    // TODO GL_STACK_OVERFLOW undeclared identifier
...
void maple::opengl_check_error(const char *filename, int lineno, const char *expr) {
...
bool maple::opengl_has_extension(const char *extension) {
    static auto exts = [] {
        GLint n = 0;
...
void maple::opengl_try_enable_debug_message() {
...
void maple::opengl_show_glfw_error_diagnose() {
    const char *errmsg;
    glfwGetError(&errmsg);
